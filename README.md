...
**[Open the browser bench](https://minh.systems/FlyLab/)** · **[Tutorial](docs/TUTORIAL.md)** · **[How to read the output](docs/INTERPRETATION.md)** · **[Documentation](docs/README.md)** · **[Research paper](papers/IJRC_FlyLab_draft.md)**
...
**The verdict this analysis returns depends on the extract it is measured on.** The repository's two committed cuts disagree: the same landscape classifies most cells composition-dominated on the sparse 1-hop `named` cut and none of them on the denser `taste_motor` cut. Run across a ladder of nested cuts of 1k to 50k cells (`flylab.analysis.scale`), the composition-dominated verdict survives on exactly one extract — the in-star — while a cut with *fewer* nodes but twenty times the mean degree is already topology-dependent. This does not isolate recurrence from density, composition or extraction recipe, and the largest rungs use only 20 permutations. A dependence result is therefore only quotable together with the structure of the graph it was measured on.
...
- [Tutorial](docs/TUTORIAL.md): seven runnable lessons, each with its real output and what it does and does not license
- [Interpretation guide](docs/INTERPRETATION.md): how to read every number the bench produces, and the misreading each one invites
