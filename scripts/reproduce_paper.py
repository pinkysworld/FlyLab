...
def save_fig(ctx: Ctx, fig, name: str, caption: str, svg: bool = False) -> None:
    ...
    ctx.captions.append((name, caption))
...
def _write_captions(ctx: Ctx) -> Path:
    ...
    for name, caption in ctx.captions:
...
def main(argv: list[str] | None = None, allow_reexec: bool = True) -> int:
    ...
    # `flylab reproduce-paper` calls main() with the subcommand still on argv.
    ...
    # `--only paper` re-renders from an existing results.json
    if only == {"paper"}:
        prev = ctx.outdir / "results.json"
        if prev.exists():
            data = json.loads(prev.read_text())
            ctx.values = data.get("values", {})
            ctx.values["_provenance"] = {"value": data.get("provenance"), "text": "", "unit": None, "note": "", "step": "provenance"}
