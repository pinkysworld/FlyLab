...
- **The central RQ2 claim reversed; substrate structure is associated with the verdict.** On the in-star `named` cut the landscape gives 11 topology-dependent / 42 composition-dominated / 31 no-effect of 84; repeated unchanged on `taste_motor` it gives 54 / 0 / 30. Across a ladder of 7 extracts (`scale_1k` … `scale_50k` plus the two committed cuts) the composition-dominated verdict occurs on **exactly one** — the in-star — and `scale_1k` has *fewer* nodes (1000 vs 1126) with mean degree 22.9 vs 1.21 and is already topology-dependent. These observations do not isolate recurrence or any other structural feature as causal. At the denser middle rungs (5k, 10k) imidacloprid is distinguishable from the three ranked edge/wiring nulls; the plain transmitter-label permutation is a separate joint target-and-sign check. The top rungs run at n = 20 (resolution 0.048), so they confirm rather than establish the saved verdict.
