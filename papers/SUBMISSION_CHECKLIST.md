...
## F. Framing: this is a computing paper

IJRC is a computing research journal, and the manuscript is written as one. The lead is the general problem — *a simulation built on a measured network and literature parameters cannot say which of its inputs its predictions depend on* — and the contribution is four transferable instruments (typed evidence propagation under a checked soundness invariant; input-dependence testing against an ordered ladder with an equivalence margin and multiplicity control; an ablation ladder scored against a matched reference; specification-family robustness with variance attribution). §3 evaluates **the instruments**: ground-truth recovery with a power surface, the permutation-convergence sweep, the matched reference distribution, the specification family, the soundness invariant, determinism and cross-backend parity. The fly pharmacology is the demonstration domain; its census, readouts and literature concordance are supporting material in §S6 and §S10.

The headline is methodological: **a dependence verdict obtained on one extract of a network can invert on another extract of the same network, and the extract's recurrence — not its size — predicts which way.** The scaling study (T27) establishes the second half: across seven extracts the composition-dominated verdict survives on exactly one, the in-star, while a *smaller* but densely recurrent extract is already topology-dependent.
...
| **B3** | The `named` cut has almost no topology, so RQ2's negatives are close to guaranteed on it — and the paper generalised from it anyway. | **The central claim is reversed, the generalisation withdrawn, and the referee's diagnosis confirmed by a scaling study.** §2.2 and T19 print both extracts' degree and recurrence statistics. The landscape is **repeated on `taste_motor`** (T12b): `dep_land_composition_dominated` of `dep_land_cells` composition-dominated on `named` against `dep_land_taste_composition_dominated` of `dep_land_taste_cells` on `taste_motor`. The same profile is then run across a ladder of **seven** nested extracts of `scale_ladder_sizes` cells (T27, `flylab.analysis.scale`): the composition-dominated verdict survives on `scale_study_n_composition_dominated` of them — the in-star — and `scale_1k`, with *fewer* nodes than `named` and twenty times the mean degree, is already topology-dependent. **Recurrence, not size, predicts the inversion**, a sharper answer than was asked for. Two caveats travel with it: the permutation budget falls with scale (`scale_study_n_range` draws), so the top rungs confirm rather than establish; and only comparisons *within* the ladder separate scale from construction. | §2.2, §3.2, §4, §5; T12, T12b, T19, T26, T27; steps `dependence`, `scale` |
...
| 3 (B3) | We do not have a defensible expectation for the seed share, which is itself the point. The conclusions do **not** hold on `taste_motor`, and the scaling study shows they hold on none of the five nested extracts either: the composition-dominated verdict is unique to the in-star. §3.2 reports that as the paper's main result. |
