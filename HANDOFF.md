...
- `analysis/dependence.py` — connectome-dependence analysis: permutation *p* first, the profile, the dependence class, and the **necessary information level** (the weakest degraded graph the test could **not** distinguish from the real cut — never "reproduces"). v0.6.1 adds a three-way verdict against a configured median-gap tolerance, a weight-matched transmitter null on the ladder's rank-3 rung, a relative effect floor, Benjamini-Hochberg across a landscape's structural tests as an exploratory sensitivity analysis, `cut_census` for the substrate's own statistics, and `synthetic_cut` / `ladder_recovery` / `ladder_power` for limited synthetic recovery checks.
- `analysis/baselines.py` — the ablation ladder: receptor-only / composition-only / topology-only / full, compared by ordering.
- `analysis/robustness.py` — 25 fixed gain specifications × 7 conclusions, and the 3×3 amplify/buffer threshold grid.
...
**Paper and pipeline**
- The manuscript is a **research article** (Introduction / Methods / Results / **Discussion** / Conclusion) organised around three research questions. Reproduction and backend parity are supporting engineering evidence, not a fourth research question. `papers/SUPPLEMENT.md` holds the mechanism rationale, null-model definitions, LIF calibration, supporting capabilities and prospective predictions. The concise revision is below IJRC's typical article-specific 6000–8000 words but within its general 5000–10000 guidance; confirm with the editor rather than pad it. The test's 5000–8000 editorial guard does not certify venue compliance. The Word preparation copy and remaining submission checks are in `papers/submission/`.
...
1. **The dependence verdict is a joint property of the prediction and the extract.** Classes differ between the sparse named and denser taste-motor cuts. Node count alone does not order the scale-ladder classes; recurrence is not isolated from density, composition or extraction recipe. The largest rungs use 20 permutations, so matching labels do not establish convergence. Report the extract, readout, concentration, permutation effort and adjustment with every dependence claim. The scale table is replayed from a saved study, not recomputed by the standard paper pipeline.
   1a. **Non-rejection is not equivalence.** The legacy within-tolerance label means a small point gap from the null median after non-rejection, not confidence-bounded statistical equivalence. Never write "reproduces" on that basis.
   ...
   1c. **The instrument has limited synthetic positive-control evidence.** Six networks per setting give imprecise detection rates, and the zero-strength control differs in cycle edges and labels. This does not establish endpoint-specific power or general false-positive calibration.
2. **The bitter-veto arm is a non-rejection, with no endpoint-specific power analysis.** Do not call it a properly powered negative or proof that wiring is unnecessary. Imidacloprid's veto ratio is undefined when MN9 is silenced.
3. **The suppression result is a property of the gain rule — and the old stability matrix was vacuous.** "A nicotinic agonist suppresses circuit activity" survives 15/25 specifications and is reversed by all 10 monotone ones, where the same drug at the same engagement *excites* the network. The v0.6 topology rows were computed at 6 shuffles (p ≤ 0.05 unattainable) **and** under a context that never rebound the gain function on the engine the permutation path resolves, so every specification fed default gains to its nulls. Recomputed at 100 shuffles with the specification pushed into the null engine and BH adjustment across 100 component tests, C3 holds 25/25 and C4 25/25 — but only 17 of C3's retentions pass the descriptive point-gap check and 8 are indeterminate. Retained by every specification: C2_rdl_disinhibition, C3_imidacloprid_topology_not_distinguishable, C4_fipronil_topology_exceeds, C7_map_bitter_veto.
4. **The ablation comparison needs its reference.** Composition-versus-full correlation is high under row normalisation and depends on shared assumptions, but exceeds the matched pseudo-compound reference 95th percentile. Jointly permuting both score vectors preserves correlation by construction and does not prove absence of compound information. Report the reference and normalisation.
5. **The uncertainty lives in two assumptions**: gain_transform, weight_threshold are the only factors whose bootstrap first-order interval excludes zero (S1 0.410 and 0.294). The largest observed negative-estimate magnitude is 0.046, set by gain_coef and **not** by the declared null factor `lif_seed` (-0.007); `drive` sits inside it and is no longer a ranked finding. Potency and Hill *n* are invisible at a saturating dose. Interactions carry 0.344 raw and 0.272 with negative estimates clipped.
...
7. **The rate and LIF engines agree on direction only** (sugar-driven MN9 ≈ 0.29 rate-model units vs ≈ 105 LIF spikes/s). That is not cross-model validation and must not be called one.
