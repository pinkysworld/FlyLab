# FlyLab handoff (v0.6.1, 2026-09-24)

Read this first if you are Claude, GPT, or another agent continuing the repo.
Owner: https://github.com/pinkysworld/FlyLab
Package version: **0.6.0** in `CITATION.cff`, `flylab/__init__.py`, `pyproject.toml` and the paper.
...
1. **The dependence verdict is a joint property of the prediction and the extract.** The saved comparisons are consistent with recurrence tracking the verdict, but they do not isolate a causal structural feature. On the 1-hop `named` cut (mean degree 1.208, 85% of edges onto four seed cells, 184 of 1126 nodes with any input) 11 of 84 cells are topology-dependent, 42 composition-dominated and 31 no-effect at n = 1000 with Benjamini-Hochberg and a 1 % relative effect floor. On `taste_motor` (mean degree 10.4) the same cells give 54 / 0 / 30. The **saved scaling study** (`papers/scale_study.json`, T27, `flylab.analysis.scale`) shows the composition-dominated verdict on **exactly one** of seven extracts, the in-star; `scale_1k` has *fewer* nodes than `named` (1000 vs 1126), mean degree 22.9, and is already topology-dependent. At 5k and 10k imidacloprid is distinguishable from all three ranked edge/wiring nulls; at 10k the separate plain transmitter-label permutation is indeterminate. At 25k and 50k the three ranked wiring nulls remain distinguishable, while the transmitter-label checks are not both distinguishable. Those rungs run at n = 20 (resolution 0.048) and so confirm rather than establish. The larger cut files and source Feathers are absent from the versioned checkout. A separate 2026-09-24 exploratory follow-up rebuilt the cuts from public v1.0 inputs and reran both confirmatory compounds at n = 100 (5k) and n = 50 (10k–50k); its eight profiles all classify topology-dependent, but n < 1000 and no across-profile correction was applied. See `data/derived/scale_study_high_power_2026-09-24.json`. The committed 1k node/edge lists match exactly and the larger cuts match T27's counts, but historical byte identity is unverified because those hashes were not recorded. "Most predictions do not need the connectome" and "the topology-dependent set is exactly the chloride-channel blockers" are **withdrawn**. Never state a dependence result without the extract, its structure, the compound, the concentration, the readout, the *p* and the correction.
...
- **Version strings are aligned at `0.6.0`.** `flylab/__init__.py`, `pyproject.toml`, `CITATION.cff` and the manuscript currently agree.
- **Open: receptor annotation and MN9 expression.** The 0.205 value is the share of graph-node × receptor-key pairs with a coarse class-level annotation across the two committed cuts, not a share of cells measured to express receptors. GEO GSE141807 is now reprocessed into VNC-wide gene-detection fractions after published QC, but no cell-class labels are joined; see `data/literature/vnc_receptor_detection_GSE141807.yaml`. A two-cell discrepancy in one replicate versus the paper is recorded and unresolved. The dataset does not identify MN9, whose proboscis motor-neuron arbors are in the brain subesophageal zone; MN9 still carries the `unknown` default weight.
...
0. **Review the scale follow-up.** `data/derived/scale_study_high_power_2026-09-24.json` has an exploratory higher-power rerun, but it is not a replacement for T27 or confirmatory evidence. The scheduled ladder step is now conditioned to run on schedule as well as manual dispatch; after the workflow change reaches the remote branch, verify its uploaded artifact and preserve input/cut hashes. Any future higher n should be budgeted from measured runtimes, with the same simple degree-preserving graph and seeded permutation procedure.
1. **v0.7 gate 7 — receptor-expression evidence.** Determine whether adult VNC motor-neuron labels can be joined defensibly to the reprocessed GEO GSE141807 barcodes; if so, report class-specific detection with annotations, sample/sex composition, detection rule and dropout treatment. Resolve MN9 separately with a class-matched brain/SEZ atlas or targeted assay. Keep gains uniform unless this produces defensible cell-class evidence.
...
5. **Housekeeping:** mint the Zenodo DOI; predictions remain prospective until a tagged release is externally archived.
