...
| **6f Substrate check** | repeat every dependence claim on a cut that is not an in-star | **done in v0.6.1 — and the verdict reversed** |
| **6i Scale ladder (new)** | the same profile across nested extracts of 1k-50k cells | **saved result in v0.6.1** — T27; exact 5k-50k cuts are absent from this checkout, so those rungs cannot currently be rebuilt |
| 7 Expression evidence | map adult VNC motor-neuron labels to reprocessed counts; resolve MN9 separately | **open — GSE141807 now has VNC-wide gene-detection fractions; cell classes and MN9 remain unresolved** |
| 7b Better power at the top of the ladder | rerun exact 5k-50k cuts with higher permutation resolution | **gated** — cuts are missing here; profile exact edge swaps before raising n |
...
1. **The dependence verdict is a joint property of the prediction and the extract; in the saved scale study, the inversion is associated with recurrence.** Over 21 compounds × 4 concentrations at n = 1000 with Benjamini-Hochberg and a 1 % relative effect floor, the 1-hop `named` cut gives 11 topology-dependent, 42 composition-dominated and 31 no-effect; repeated unchanged on the denser `taste_motor` cut it gives 54 / 0 / 30. The saved scaling study over seven extracts (T27) shows the composition-dominated verdict surviving on exactly one of them — the in-star — while `scale_1k`, with *fewer* nodes than `named` and twenty times the mean degree, is already topology-dependent. This is consistent with recurrence tracking the change, but does not isolate a causal structural feature; the larger input cuts are absent from this checkout. **"Most predictions do not need the connectome" is withdrawn**, and so is "exactly the chloride-channel blockers". Report the extract's structure (T19, T27) with any dependence claim.
...
1. **Expression evidence.** 0.205 is the fraction of graph-node × receptor-key pairs with a coarse class-level annotation across the two committed cuts, not a fraction of cells measured to express receptors. GEO GSE141807 is reprocessed into gene-detection fractions after published QC, but this is VNC-wide and has no joined cell-class labels. Determine whether adult motor-neuron annotations can be matched to those barcodes; the data do not identify MN9, whose proboscis motor-neuron arbors are in the brain subesophageal zone. Resolve MN9 with a class-matched brain/SEZ atlas or a targeted assay.
