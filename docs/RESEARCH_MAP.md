...
| 6b Connectome dependence | permutation *p*, dependence class, necessary information level | **done in v0.6**; v0.6.1 adds a three-way descriptive median-gap diagnostic, a weight-matched transmitter null, a relative effect floor, BH-adjusted component tests in an exploratory landscape and ground-truth validation of the ladder |
...
| **6i Scale ladder (new)** | the same profile across nested extracts of 1k-50k cells | **done in v0.6.1** — the observed class remains topology-dependent across the tested clean rungs; recurrence is not isolated causally (T27) |
| 7 Expression coverage | adult motor-neuron receptor expression, or a declared bound | **open — 0.205, MN9 unresolved** |
| 7b Better power at the top of the ladder | the 25k/50k rungs run at n = 20 (resolution 0.048) | **open** — they confirm the settled verdict rather than establishing it |
...
1. **The dependence verdict is a joint property of the prediction and the extract, with recurrence a candidate associated feature.** Over 21 compounds × 4 concentrations at n = 1000 with Benjamini-Hochberg and a 1 % relative effect floor, the 1-hop `named` cut gives 11 topology-dependent, 42 composition-dominated and 31 no-effect; repeated unchanged on the denser `taste_motor` cut it gives 54 / 0 / 30. The scaling study over seven extracts (T27) shows the composition-dominated verdict surviving on exactly one of them — the in-star — while `scale_1k`, with *fewer* nodes than `named` and twenty times the mean degree, is already topology-dependent. **"Most predictions do not need the connectome" is withdrawn**, and so is "exactly the chloride-channel blockers". Report the extract's structure (T19, T27) with any dependence claim.
2. **The bitter-veto arm is a non-rejection without endpoint-specific power calibration.** At n = 300 on the taste-motor cut, fipronil's veto ratio gives p = 0.42 / 0.88 / 0.20 / 0.24 with all |z| < 0.25, replacing the ten-shuffle hand-wave of v0.5. H6 has no demonstrated wiring dependence on this endpoint; wiring independence is not established. Imidacloprid's veto ratio there is **undefined** (MN9 silenced), not zero.
